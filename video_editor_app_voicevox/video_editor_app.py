...
if uploaded_file:
    ...
    # 🌟【新機能】テキストファイル・CSVからテロップを一括インポートするエリア
    st.write("---")
    st.write("### 📥 テロップ・原稿の一括インポート（1行＝1ページ）")
    st.info("💡 メモ帳やExcel（CSV）で作成した文章を貼り付けると、改行（行番号）を元に各ページへ一気に流し込めます。")
    
    import_text = st.text_area("ここに1行ずつ文章を貼り付けてください（例：1行目が1ページ目、2行目が2ページ目に反映されます）", height=150)
    ...
    if st.button("⚡ 貼り付けた文章を全ページに一括反映する"):
        if import_text.strip():
            ...
            for idx, line in enumerate(lines):
                if idx < total_pages: # PDFの最大ページ数を超えないようにディフェンス
            ...
            st.success(f"🎉 成功！ 最大 {len(lines)} ページ分にテロップを一括反映しました。")
            st.rerun()
        else:
            st.warning("文章が空っぽです。")
        ...
        with col_right:
            ...
            if st.button("🔊 このページの音声を試聴する", key=f"btn_{current}"):
                if voice_input.strip():
                    with st.spinner("VOICEVOXが喋っています..."):
                        try:
                            base_url = "http://localhost:50021"
                            speaker_id = 2
                            res1 = requests.post(f"{base_url}/audio_query", params={"text": voice_input, "speaker": speaker_id})
                            query_data = res1.json()
                            res2 = requests.post(f"{base_url}/synthesis", params={"speaker": speaker_id}, data=json.dumps(query_data))
                            st.audio(res2.content, format="audio/wav")
                        except Exception as e:
                            st.error("VOICEVOXサーバーに接続できません。アプリが起動しているか確認してください。")
                else:
